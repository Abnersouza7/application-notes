...
> **Draft, revision 0.3.** The specification content in this document is sourced and
> verified, and the procedure in Section 6 has now been run end to end against an I3C CMIS
> target. Section 9 carries the output. One item is still outstanding, in-band interrupt
> delivery, because the target on the bench does not implement it. This revision is for
> internal review and is not for publication.
...
target wraps inside the segment. That is one target answering for itself. It is not
evidence about any other, which is the reason the command exists.
...
static 0x00 -> dynamic 0x09  BCR 0x20  DCR 0x00
```
...
**In-band interrupts, outstanding.** ENEC is accepted and the listen path runs, but this
target advertises BCR 0x20, which does not claim the in-band interrupt capability, and it
never raises one. Interrupt delivery is therefore still unmeasured, and Section 7 remains
...
ENEC sent to 0x09, listening for 10 s
0 in-band interrupt(s) in 10 s
```

**Independent confirmation on the wire.** A host adapter reports what it believes it
received, which is not the same thing as what the target drove. We captured the bus with a
logic analyzer and replayed the decoded transactions against the CMIS register model:
