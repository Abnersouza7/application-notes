# AN0002 verification images

Two small firmware images for the NUCLEO-H503RB. They exist so that a firmware
update performed over I2C can be confirmed from outside the programming tool:
flash one, look at the serial output, flash the other, and watch it change.

| File | Description |
|---|---|
| `an0002_image_a.bin` | Prebuilt Image A, slow LED blink |
| `an0002_image_b.bin` | Prebuilt Image B, fast LED blink |
| `main.c` | Source for both images |
| `link.ld` | Linker script, 128 KB flash at 0x08000000, 32 KB SRAM |
| `Makefile` | Builds both images |

They are 864 and 860 bytes, and both are linked to run from `0x08000000`.
...
The images are the same programs used in AN0001, retargeted to name I2C and the
Pulsar in the banner. Nothing in them depends on the bus used to load them, so
they serve equally well when the target is programmed from a Supernova.
...
Note that neither pin collides with `PB3` and `PB4`, the pins the system
bootloader uses for I2C on this part. The images can therefore be loaded and
observed without moving any wiring.
...
MIT, the same as the rest of the AN0002 example code. See `../LICENSE`.
