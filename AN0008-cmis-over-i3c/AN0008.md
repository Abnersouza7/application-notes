...
company: Binho Inc.
website: binho.io
keywords: AN0008, Supernova, CMIS, OIF, I3C, MIPI I3C, SDR, I3CMCI, optical module, OSFP-XD, ENTDAA, IBI, in-band interrupt, hot-join, dynamic address assignment
trademarks: |
...
> verified. The procedure in Section 6 has not yet been executed against an I3C CMIS
> target, and Section 9 states exactly what is outstanding. This revision is for internal
...
AN0007 covers the register model itself and is a prerequisite. This document assumes it.
...
| Item | Role | Basis |
|---|---|---|
| Binho Supernova | Host adapter, I3C controller | Not yet exercised for this note |
| NXP MCX N947 running AN15071 firmware | I3C CMIS target | Not yet exercised for this note |
| Any I3C CMIS target at static address 0x50 | Target | Not yet exercised for this note |
...
> **No row in this table has been exercised on hardware at revision 0.1.** The mapping in
> Section 4 is derived from OIF-CMIS-05.3 and 05.4, from MIPI I3C Basic v1.1.1, and from
> OSFP-XD Revision 1.1. It becomes revision 1.0 when it has been run against a target.
...
The utility supplied with this note is the same file supplied with AN0007, AN0009 and
AN0010. `--transport i3c` selects the I3C binding.

The Supernova drives `PUSH_PULL_12_5_MHZ_50_DC`, which is exactly the rate OSFP-XD names,
so the bring-up below runs at the rate an OSFP-XD host would use rather than at a
...
An eight byte read at `00h:124` tells them apart, because bytes 4 through 7 come back
either as `00h:0-3` or as the start of Page 00h:
...
The command performs the AN0007 sequence over I3C single data rate private transfers and
...
**Offline.** The register access layer this note drives is the one AN0007 self-checks, 7 of
7 against a simulated module. That covers the address arithmetic, not the I3C transport.

**On hardware.** Nothing in this section has been measured. Outstanding at revision 0.1:
dynamic address assignment against a real target, the CMIS bring-up over single data rate
private transfers, the roll-over determination, and in-band interrupt delivery.
...
| Interrupts never arrive | ENEC was not accepted, or the target does not implement in-band interrupts |
