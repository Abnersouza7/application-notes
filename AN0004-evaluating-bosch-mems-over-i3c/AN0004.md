...
Assigning addresses at run time also relaxes a constraint that applies to these parts on a
static-address bus. The BMP581 datasheet gives its I2C address as `0x46` for SDO at logic 0
and `0x47` for SDO at logic 1, and the BMP585 uses the same pair. Two such parts therefore
share a static-address bus without difficulty, one at each address, but a third cannot be
addressed at all. Under ENTDAA the number of parts sharing an address pair on paper stops
being a constraint, because each is assigned a distinct dynamic address during enumeration.

> **Note:** the address limit above is taken from the BMP581 and BMP585 datasheets and was not
> exercised for this document. The I3C Target Board has one Shuttle 3.0 socket, so the two
...
One row needs its test stated, because the obvious test for it gives the wrong answer. A group
address is a write destination: it exists so a controller can address several targets at once,
which means a read from one has no defined answer and a target may refuse it while implementing
group addressing correctly. Asking whether the target answers a read at the group address
therefore cannot distinguish a part that lacks the feature from one that has it. The test used
here writes a register through the group address and reads that register back at the target's
own dynamic address, and requires three results: the write must be refused before SETGRPA, land
while the group address is assigned, and be refused again after RSTGRPA. The trial value is
first written at the target's own address to confirm the register will hold it, so that a
refused group write cannot be confused with a register that ignores the value.

All three parts refuse the write at every stage, at three different group addresses, while
SETGRPA itself returns a successful result each time. This is the same pattern Section 5.5
describes and not an additional finding.
...
The verdict counts were 11 supported, 7 not implemented and 7 undetermined for all three parts,
and each part was measured twice to confirm the counts are reproducible.
...
| 1.1 | 31 August 2026 | Section 5.6: the SETGRPA and RSTGRPA row is re-measured with a write through the group address rather than a read from it, and the test is now stated. The verdict is unchanged for all three parts. Section 8.2: the BMI323 verdict counts are corrected to 11 supported, 7 not implemented and 7 undetermined. The previous counts were taken from a run in which the part was still holding a mode left behind by an earlier run, which reported SETXTIME as undetermined; the utility now clears that state and the counts are reproducible across runs. |
| 1.2 | 1 September 2026 | No change to the text. The bundled utility is updated to version 1.2, which fixes two faults that stop in-band interrupts being reported from some targets: an interrupt payload larger than the adapter accepts was discarded with no error, and the measurement discarded the first interrupt without clearing its source, which stalls any target whose interrupt is a level the host must clear. Neither fault affects the BMI323, BMP581 or BMP585, whose measured results are unchanged. |
